...
4. **CLI** (`cli/`). Links a separate device key with phone approval, then runs as the same account. Phone-composed text can be handed off to a reachable CLI over `/qop/sync/1`; the CLI persists it in `messages.db` and retries while it runs.
