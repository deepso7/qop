...
describe("local registration", () => {
  ...
  it("treats unscoped registration state as absent", async () => {
    const original = loadRegistration();
    await Effect.runPromise(original.startLocalRegistration("ABC-123"));
    ...
    secureStoreMock.items.set("qop.registration.v2", encoded ?? "");

    await expect(
      Effect.runPromise(loadRegistration().loadLocalRegistration())
    ).resolves.toBeNull();
