...
Set `EXPO_PUBLIC_API_URL`, `EXPO_PUBLIC_RPC_URL`, `EXPO_PUBLIC_REGISTRY_ADDRESS`, `EXPO_PUBLIC_REGISTRY_CHAIN_ID`, and `EXPO_PUBLIC_RELAY_ADDRS` before running the app. `EXPO_PUBLIC_RELAY_ADDRS` is a comma-separated list of minip2p relay multiaddresses. The registry values pin the EIP-712 domain the local owner key may authorize, and the RPC URL lets the app read registrations and device membership directly from the chain. The API is used for invitation-gated registration and later phone-approved add/remove device actions. Native devices and emulators must use API and RPC addresses that can reach the development machine; `127.0.0.1` only works when those services are available inside that device's network namespace.

Local registration records are scoped to the configured chain and registry address. Switching registries requires registration again; the existing identity keys and conversation history remain on the device.

Chat authorization is checked against the registry on first use of each transport connection, in both directions, then reused for at most 60 seconds of monotonic running time. Resume from background invalidates that cache. Disconnecting clears verification; reconnecting requires a fresh registry check before sending or accepting chat messages. Profile → Devices links additional keys (including a CLI) and can remove them without deleting conversation history. Phone↔CLI outbox sync is a later milestone.
